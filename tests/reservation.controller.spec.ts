...
describe("Reservation Controller", () => {
	...
	const mockReservation = {
		...
		name: "User Registration Reservation",
		description: "Reservation for user registration forms",
		type: "email",
		createdAt: new Date(),
		updatedAt: new Date(),
	};
	...
	const mockReservations = [
		...
		},
		{
			...
			description: "Reservation for SMS notifications",
			type: "sms",
			createdAt: new Date(),
			updatedAt: new Date(),
		},
		{
			...
			description: "Reservation for email marketing campaigns",
			type: "email",
			createdAt: new Date(),
			updatedAt: new Date(),
		},
		{
			...
			description: "Reservation without type",
			type: null,
			createdAt: new Date(),
			updatedAt: new Date(),
		},
	...
	beforeEach(() => {
		prisma = {
			reservation: {
				findMany: async (_params: Prisma.ReservationFindManyArgs) => {
					// Return multiple reservations for grouping tests
					if (req.query?.groupBy) {
						return mockReservations;
					}
					return [mockReservation];
				},
				count: async (_params: Prisma.ReservationCountArgs) => {
				...
				delete: async (params: Prisma.ReservationDeleteArgs) => ({
					...mockReservation,
					id: params.where.id,
				}),
			},
	...
	describe(".getAll()", () => {
		...
		it("should group reservations by type field", async function () {
			this.timeout(TEST_TIMEOUT);
		...
		it("should group reservations by name field", async function () {
			this.timeout(TEST_TIMEOUT);
			...
			expect(sentData.data.grouped).to.have.property("User Registration Reservation");
			expect(sentData.data.grouped).to.have.property("SMS Notification Reservation");
		});

		it("should handle reservations with null values in grouping field", async function () {
			this.timeout(TEST_TIMEOUT);
			req.query = { groupBy: "type" };
			await reservationController.getAll(req as Request, res, next);
			expect(statusCode).to.equal(200);
	...
	describe(".create()", () => {
		it("should create a new reservation", async function () {
			...
			const createData = {
				...
				description: "Reservation for contact forms with validation",
			};
			req.body = createData;
		...
		it("should create a new reservation with type field", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Email Reservation",
				description: "Reservation for email notifications",
				type: "email",
			};
			req.body = createData;
			await reservationController.create(req as Request, res, next);
		...
		it("should create a new reservation without type field", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Generic Reservation",
				description: "Reservation without type",
		...
		it("should handle form data (multipart/form-data)", async function () {
			...
			const createData = {
				...
				description: "Reservation from form data",
				type: "form",
			};
			req.body = createData;
		...
		it("should handle form data (application/x-www-form-urlencoded)", async function () {
			...
			const createData = {
				...
				description: "Reservation from URL encoded data",
			};
			req.body = createData;
			(req as any).get = (header: string) => {
				if (header === "Content-Type") {
		...
		it("should handle validation errors", async function () {
			...
			const createData = {
				name: "",
				description: "Reservation with empty name",
			};
			req.body = createData;
			await reservationController.create(req as Request, res, next);
			expect(statusCode).to.equal(400);
		...
		it("should handle Prisma errors", async function () {
			...
			const createData = {
				...
				description: "Reservation that will cause Prisma error",
			};
			req.body = createData;
		...
		it("should handle internal errors", async function () {
			...
			const createData = {
				name: "Test Reservation",
				description: "Reservation that will cause internal error",
			};
			req.body = createData;
	...
	describe(".update()", () => {
		it("should update reservation details", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				name: "Enhanced Contact Form Reservation",
				description: "Updated reservation with additional validation and styling options",
			};
			req.params = { id: mockReservation.id };
			req.body = updateData;
		...
		it("should update reservation type field", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				type: "sms",
			};
		...
		it("should update multiple reservation fields including type", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				name: "Updated Email Reservation",
		...
		it("should handle non-existent reservation update", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				name: "Updated Reservation",
			};
	...
	describe("Edge Cases and Integration", () => {
		...
		it("should handle very long reservation name", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "A".repeat(1000), // Very long name
				description: "Reservation with very long name",
			};
			req.body = createData;
			await reservationController.create(req as Request, res, next);
			expect(statusCode).to.equal(201);
		...
		it("should handle special characters in reservation data", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Reservation with special chars: !@#$%^&*()",
		...
		it("should handle concurrent requests", async function () {
			...
			const createData = {
				...
				description: "Reservation created concurrently",
			};
			req.body = createData;
			...
			const promises = Array(5)
				.fill(null)
				.map(() => reservationController.create(req as Request, res, next));
...
describe("Data Grouping Helper", () => {
	const testData = [
		{ id: 1, name: "Reservation 1", type: "email", category: "marketing" },
		{ id: 2, name: "Reservation 2", type: "sms", category: "notification" },
		{ id: 3, name: "Reservation 3", type: "email", category: "marketing" },
		{ id: 4, name: "Reservation 4", type: null, category: "general" },
		{ id: 5, name: "Reservation 5", type: "push", category: "notification" },
	];
	...
	describe("groupDataByField()", () => {
		...
		it("should handle undefined values by placing them in unassigned group", () => {
			const dataWithUndefined = [
				{ id: 1, name: "Reservation 1", type: "email" },
				{ id: 2, name: "Reservation 2" }, // missing type field
			];
		...
		it("should group by string values correctly", () => {
			...
			expect(result).to.have.property("Reservation 1");
			expect(result).to.have.property("Reservation 2");
			expect(result).to.have.property("Reservation 3");
			expect(result).to.have.property("Reservation 4");
			expect(result).to.have.property("Reservation 5");
			expect(result["Reservation 1"]).to.have.length(1);
		});
	});
});
