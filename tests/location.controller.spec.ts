import { controller } from "../app/location/location.controller";
...
describe("Location Controller", () => {
	...
	const mockLocation = {
		...
		name: "User Registration Location",
		description: "Location for user registration forms",
		type: "email",
		createdAt: new Date(),
		updatedAt: new Date(),
	};
	...
	const mockLocations = [
		...
		},
		{
			...
			name: "SMS Notification Location",
			description: "Location for SMS notifications",
			type: "sms",
			createdAt: new Date(),
			updatedAt: new Date(),
		},
		{
			...
			name: "Email Marketing Location",
			description: "Location for email marketing campaigns",
			type: "email",
			createdAt: new Date(),
			updatedAt: new Date(),
		},
		{
			...
			description: "Location without type",
			type: null,
			createdAt: new Date(),
			updatedAt: new Date(),
		},
	...
	beforeEach(() => {
		prisma = {
			location: {
				findMany: async (_params: Prisma.LocationFindManyArgs) => {
					// Return multiple locations for grouping tests
					if (req.query?.groupBy) {
						return mockLocations;
					}
					return [mockLocation];
				},
				count: async (_params: Prisma.LocationCountArgs) => {
				...
				findFirst: async (params: Prisma.LocationFindFirstArgs) =>
					params.where?.id === mockLocation.id ? mockLocation : null,
				findUnique: async (params: Prisma.LocationFindUniqueArgs) =>
					params.where?.id === mockLocation.id ? mockLocation : null,
				create: async (params: Prisma.LocationCreateArgs) => ({
					...mockLocation,
					...params.data,
				}),
				update: async (params: Prisma.LocationUpdateArgs) => ({
					...mockLocation,
					...params.data,
				}),
				delete: async (params: Prisma.LocationDeleteArgs) => ({
					...mockLocation,
					id: params.where.id,
				}),
			},
		...
		locationController = controller(prisma as PrismaClient);
		sentData = undefined;
		statusCode = 200;
		req = {
			query: {},
	...
	describe(".getAll()", () => {
		it("should return paginated locations", async function () {
			this.timeout(TEST_TIMEOUT);
			req.query = { page: "1", limit: "10" };
			await locationController.getAll(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
			expect(sentData).to.have.property("data");
		});

		it("should group locations by type field", async function () {
			this.timeout(TEST_TIMEOUT);
		...
		it("should group locations by name field", async function () {
			this.timeout(TEST_TIMEOUT);
			...
			expect(sentData.data.grouped).to.have.property("User Registration Location");
			expect(sentData.data.grouped).to.have.property("SMS Notification Location");
		});

		it("should handle locations with null values in grouping field", async function () {
			this.timeout(TEST_TIMEOUT);
			req.query = { groupBy: "type" };
			await locationController.getAll(req as Request, res, next);
			expect(statusCode).to.equal(200);
	...
	describe(".getById()", () => {
		it("should return a location", async function () {
			this.timeout(TEST_TIMEOUT);
			req.params = { id: mockLocation.id };
			await locationController.getById(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
			expect(sentData).to.have.property("data");
			expect(sentData.data).to.deep.include({ id: mockLocation.id });
		});
	...
	describe(".create()", () => {
		it("should create a new location", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Contact Form Location",
				description: "Location for contact forms with validation",
			};
			req.body = createData;
			await locationController.create(req as Request, res, next);
		...
		it("should create a new location with type field", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Email Location",
				description: "Location for email notifications",
				type: "email",
			};
			req.body = createData;
			await locationController.create(req as Request, res, next);
		...
		it("should create a new location without type field", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Generic Location",
				description: "Location without type",
		...
		it("should handle form data (multipart/form-data)", async function () {
			...
			const createData = {
				name: "Form Location",
				description: "Location from form data",
				type: "form",
			};
			req.body = createData;
		...
		it("should handle form data (application/x-www-form-urlencoded)", async function () {
			...
			const createData = {
				name: "URL Location",
				description: "Location from URL encoded data",
			};
			req.body = createData;
			(req as any).get = (header: string) => {
				if (header === "Content-Type") {
		...
		it("should handle Prisma errors", async function () {
			...
			const createData = {
				name: "Test Location",
				description: "Location that will cause Prisma error",
			};
			req.body = createData;

			// Mock Prisma to throw an error
			prisma.location.create = async () => {
				const error = new Error("Database connection failed") as any;
				error.name = "PrismaClientKnownRequestError";
				error.code = "P1001";
				throw error;
		...
		it("should handle internal errors", async function () {
			...
			const createData = {
				name: "Test Location",
				description: "Location that will cause internal error",
			};
			req.body = createData;
	...
	describe(".update()", () => {
		it("should update location details", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				name: "Enhanced Contact Form Location",
				description: "Updated location with additional validation and styling options",
			};
			req.params = { id: mockLocation.id };
			req.body = updateData;
			await locationController.update(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
			expect(sentData).to.have.property("data");
			expect(sentData.data).to.have.property("id");
		...
		it("should update location type field", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				type: "sms",
			};
		...
		it("should update multiple location fields including type", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				name: "Updated Email Location",
		...
		it("should handle non-existent location update", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				name: "Updated Location",
			};
	...
	describe(".remove()", () => {
		it("should delete a location", async function () {
			this.timeout(TEST_TIMEOUT);
			req.params = { id: mockLocation.id };
			await locationController.remove(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
		});
		...
		it("should handle non-existent location deletion", async function () {
			this.timeout(TEST_TIMEOUT);
			req.params = { id: "507f1f77bcf86cd799439099" };
			await locationController.remove(req as Request, res, next);
			expect(statusCode).to.equal(404);
	...
	describe("Edge Cases and Integration", () => {
		...
		it("should handle very long location name", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "A".repeat(1000), // Very long name
				description: "Location with very long name",
			};
			req.body = createData;
			await locationController.create(req as Request, res, next);
			expect(statusCode).to.equal(201);
		...
		it("should handle special characters in location data", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
		...
		it("should handle concurrent requests", async function () {
			...
			const createData = {
				...
				description: "Location created concurrently",
			};
			req.body = createData;
			...
			const promises = Array(5)
				.fill(null)
				.map(() => locationController.create(req as Request, res, next));
...
describe("Data Grouping Helper", () => {
	const testData = [
		{ id: 1, name: "Location 1", type: "email", category: "marketing" },
		{ id: 2, name: "Location 2", type: "sms", category: "notification" },
		{ id: 3, name: "Location 3", type: "email", category: "marketing" },
		{ id: 4, name: "Location 4", type: null, category: "general" },
		{ id: 5, name: "Location 5", type: "push", category: "notification" },
	];
	...
	describe("groupDataByField()", () => {
		...
		it("should handle undefined values by placing them in unassigned group", () => {
			const dataWithUndefined = [
				{ id: 1, name: "Location 1", type: "email" },
				{ id: 2, name: "Location 2" }, // missing type field
			];
		...
		it("should group by string values correctly", () => {
			...
			expect(result).to.have.property("Location 1");
			expect(result).to.have.property("Location 2");
			expect(result).to.have.property("Location 3");
			expect(result).to.have.property("Location 4");
			expect(result).to.have.property("Location 5");
			expect(result["Location 1"]).to.have.length(1);
		});
	});
});
