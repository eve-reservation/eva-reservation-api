...
export const router = (route: Router, controller: IController): Router => {
	 ...
	 *     description: Retrieve a specific location by its unique identifier with optional field selection
	 *     tags: [Location]
	 *     security:
	 *       - bearerAuth: []
	 *     parameters:
	 ...
	 *         description: Location ID (MongoDB ObjectId format)
	 ...
	 */
	// Cache individual location with predictable key for invalidation
	routes.get(
		"/:id",
		cache({
			ttl: 90,
			keyGenerator: (req: Request) => {
				...
				return `cache:location:byId:${req.params.id}:${fields}`;
			},
		}),
		controller.getById,
	);
	 ...
	 *     description: Retrieve locations with advanced filtering, pagination, sorting, field selection, and optional grouping
	 *     tags: [Location]
	 *     security:
	 *       - bearerAuth: []
	 *     parameters:
	 ...
	 *           type: string
	 *           enum: ["true"]
	 *         description: Include location documents in response
	 *       - in: query
	 *         name: pagination
	 *         required: false
	 *         schema:
	 ...
	 *         $ref: '#/components/responses/InternalServerError'
	 */
	// Cache location list with predictable key for invalidation
	routes.get(
		"/",
		cache({
			ttl: 60,
			keyGenerator: (req: Request) => {
				...
				return `cache:location:list:${queryKey}`;
			},
		}),
		controller.getAll,
	);
	 ...
	 *                 example: "Email Welcome Location"
	 *               description:
	 *                 type: string
	 *                 description: Location description
	 *                 example: "Welcome email location for new users"
	 *               type:
	 *                 type: string
	 *                 enum: ["email", "sms", "push", "form"]
	 *                 description: Location type for categorization
	 *                 example: "email"
	 *               isDeleted:
	 *                 type: boolean
	 ...
	 *     description: Update location data by ID (partial update)
	 *     tags: [Location]
	 *     security:
	 *       - bearerAuth: []
	 *     parameters:
	 ...
	 *                 minLength: 1
	 *                 description: Location name
	 *                 example: "Updated Email Location"
	 *               description:
	 *                 type: string
	 *                 description: Location description
	 *                 example: "Updated description for the location"
	 *               type:
	 *                 type: string
	 *                 enum: ["email", "sms", "push", "form"]
	 *                 description: Location type for categorization
	 ...
	 *     description: Permanently delete a location by ID
	 *     tags: [Location]
	 *     security:
	 *       - bearerAuth: []
	 *     parameters:
